...
int Tcp::initialize() {
    ...
    if (this->isServer) {
        ...
        if (connect(this->socketDescriptor,
                    (struct sockaddr *) &sin, sizeof(sin)) < 0) {
            //return an error represent error at this method
            cout << "error in the connect client part" << endl;
...
***********************************************************************/
int Tcp::reciveData(char* buffer, int size, int descriptor) {
...
int Tcp::tcpAccept() {
    ...
    int newClientSd = accept(this->socketDescriptor,
                             (struct sockaddr *) &client_sin, &addr_len);
    if (newClientSd < 0) {
        cout << "in error accept, trying again  " << newClientSd << endl;
        //return an error represent error at this method
        return newClientSd;
    }
    return newClientSd;
